...
def take_next_step(grid, ongoing_paths):
  ...
  while ongoing_paths:
    ...
    trailheads = ongoing_paths.pop(position)
    i, j = position
    if grid[i][j] == 9:
      trail_sum += len(trailheads)
    else:
      next_steps = get_next_steps(grid, position)
      for next_step in next_steps:
        ongoing_paths[next_step].update(trailheads)
  return trail_sum
...
def solution():
  ...
  ongoing_paths = defaultdict(set)
  for position in trailheads:
    ongoing_paths[position].add(position)
